...
class TransactionService:
    ...
    def analyze_spending(self, db: Session, user_id: int, month: int, year: int):
        current_spent = db.query(func.sum(Transaction.amount)).join(Category).filter(
            Transaction.user_id == user_id,
        ...
        previous_spent = db.query(func.sum(Transaction.amount)).join(Category).filter(
            Transaction.user_id == user_id,
            Category.type == 'EXPENSE',
            extract('month', Transaction.transaction_date) == last_month,
            extract('year', Transaction.transaction_date) == last_year,
            Transaction.deleted_at == None
        ).scalar() or 0

        diff_percent = 0
        if previous_spent > 0:
            diff_percent = ((current_spent - previous_spent) / previous_spent) * 100
        ...
        advice = "Tình hình tài chính của bạn đang rất ổn định."
        if diff_percent > 20:
            advice = f"Cảnh báo: Bạn đang tiêu nhiều hơn tháng trước {diff_percent:.1f}%. Hãy cắt giảm bớt {top_category[0] if top_category else ''} nhé!"
        elif current_spent > 0 and previous_spent == 0:
            advice = "Tháng đầu tiên sử dụng app, hãy cố gắng duy trì thói quen ghi chép nhé!"

        return {
            "current_month_total": float(current_spent),
            "previous_month_total": float(previous_spent),
            "increase_percent": round(diff_percent, 2),
            "top_expense_category": top_category[0] if top_category else "Chưa có",
            "ai_advice": advice
        }